$(function(){
    ...
    const address = $('.ending>.creater>address>a');
    const $replay = $('.ending>.link>.replay');
    ...
    function resizing(){
        ...
        if(winWidth<1920){
        alert('해당 이벤트 페이지는 PC(1920px)에 최적화되었습니다.');
        }
    }

    $(window).one('resize', function(){
        clearTimeout(media);
        media = setTimeout(resizing,400);
    });
    ...
    address.on('click',function(evt){
        evt.preventDefault();
        evt.stopPropagation();
        $('.ending>.creater>address>ul').fadeIn(500);
    })
    $('body').on('click',function(){
        $('.ending>.creater>address>ul').fadeOut(500);
    })
    // $replay.on('click',function(evt){
    //     location.reload();
    // })
